...
- [x] Create the Vite React + Tailwind app using the same dependency versions as `day094`.
- [x] Configure `base: "./"` in `vite.config.js` so `dist/` works from `/day095/`.
- [x] Add Tailwind tokens for `ledger`, `ink`, `teal`, `steel`, `copper`, `mint`, `amber`, `grid`, and `slip`.
- [x] Copy only app-consumed raster assets into `day095/public-optimized/assets/`:
...
- [x] Add `siteData.js` with nav items, page metadata, KPIs, alerts, features, pricing plans, FAQ, posts, news, and case studies.
- [x] Run `npm install`.
- [x] Run `npm run build`.
...
- [x] Implement hash-based routing for `/`, `/diagnosis`, `/demo`, `/pricing`, `/contact`, `/features`, `/channels`, `/cases`, `/security`, `/help`, `/faq`, `/news`, `/column`, and `/legal`.
- [x] Build a sticky header matching the mockups: logo mark, compact nav, login text, teal CTA.
- [x] Build reusable `Button`, `SectionShell`, `PaperCard`, `Badge`, `KpiCard`, `LabelSticker`, `BottomCta`, and `PageHero` components.
- [x] Implement `Icons.jsx` as inline SVGs matching the generated transparent icons, with transparent backgrounds and `currentColor` support.
- [x] Implement `Dashboard.jsx` as live HTML/CSS, not a static dashboard image, matching the hero dashboard mockup: sidebar, KPI row, alert table, channel cards.
- [x] Add CSS animations: page reveal, dashboard line draw, card stagger, CTA pulse, subtle label float, reduced-motion fallback.
- [x] Run `npm run build`.
...
- [x] Recreate the full LP mockup structure:
  ...
  - FAQ preview
  - final teal CTA band
- [x] Use the mockup Japanese copy as live text:
  - `小さなEC運営を、静かな管制室に変える。`
  - `在庫・価格・SKUを、毎朝見える状態へ。`
  - `無料で棚卸し診断を受ける`
  - `デモ画面を見る`
- [x] Place decorative logistics labels/stamps from generated assets where the mockup uses them.
- [x] Ensure desktop width resembles the 864x1821 LP reference while remaining responsive.
- [x] Run `npm run build`.
...
- [x] Implement `/diagnosis` matching `page-free-diagnosis.png`: page hero, 3-step progress, sales channel checkboxes, SKU/monthly order inputs, issue radio chips, textarea, diagnosis preview.
- [x] Implement `/demo` matching `page-demo.png`: dashboard hero, tabs, annotated screen modules, component descriptions.
- [x] Implement `/pricing` matching `page-pricing.png`: three pricing cards, comparison table, FAQ strip, CTA.
- [x] Implement `/contact` matching `page-contact.png`: support flow, contact form, contact methods, response-time note.
- [x] Use live HTML form controls. No submission backend is required; use local interaction state only.
- [x] Run `npm run build`.
...
- [x] Implement `/features` matching `page-features.png`.
- [x] Implement `/channels` matching `page-channels.png`, using generic channel marks and no exact official logos.
- [x] Implement `/cases` matching `page-case-studies.png`, using the generated case study photo thumbnails.
- [x] Implement `/security` matching `page-security.png`, including data flow, information cards, permission matrix, and security icon row.
- [x] Run `npm run build`.
...
- [x] Implement `/help` matching `page-help-center.png`.
- [x] Implement `/faq` matching `page-faq.png`, with interactive accordion state.
- [x] Implement `/news` matching `page-news.png`.
- [x] Implement `/column` matching `page-column.png`.
- [x] Implement `/legal` matching `page-legal-policy.png`, with left-side document navigation and text-heavy content.
- [x] Run `npm run build`.
...
- [x] Tune desktop layout at 1440px, tablet at 1024px, and mobile at 390px.
- [x] Ensure no text overlaps in nav, buttons, cards, dashboard tables, pricing cards, and forms.
- [x] Add hover/focus states, keyboard-visible focus, `aria-label`s where icon-only controls exist.
- [x] Confirm images have useful `alt` or `aria-hidden` when decorative.
- [x] Keep card radii at 8px or less unless the mockup uses a pill control.
- [x] Run `npm run build`.
...
- [x] Register Day095 in root `script.js` with title, category, industry, focus, summary, metric, tech, and URL.
- [x] Run `node --check script.js`.
- [x] Run `npm run build` in `day095/`.
- [x] Start local dev server with `npm run dev -- --port 8095`.
- [x] Use Browser Use (`browser-client` with `iab`) to inspect:
  - desktop `http://127.0.0.1:8095/day095/`
  - mobile 390px width
  - at least three lower pages
- [x] Capture screenshots to `day095/docs/qa/`.
- [x] Fix any layout, console, or accessibility issue found during browser review.
- [x] Create implementation summary with commands run and known limitations.
