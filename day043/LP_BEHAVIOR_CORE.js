(function () {
  ...
  const reservationUrl =
    config.reservationUrl || "https://timerex.net/s/bodymake_tokyo_yuta";
  const pdfDownloadUrl =
    config.pdfDownloadUrl || "./downloads/food-checklist.pdf";
  const leadEndpoint =
    config.leadEndpoint || "https://formsubmit.co/ajax/contact@bodymake-yuta.com";
  ...
  function initPdfForm(utm) {
    ...
    const formAction = (pdfForm.getAttribute("action") || "").trim();
    ...
    pdfForm.addEventListener("submit", async function (event) {
      ...
      try {
        let response;
        const isFormSubmit = leadEndpoint.indexOf("formsubmit.co") !== -1;
        ...
        if (
          responseJson &&
          typeof responseJson.success !== "undefined" &&
