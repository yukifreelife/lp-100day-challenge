...
// Step Form - 2ステップフォーム
// ===================================
const stepForm1 = document.getElementById('contactFormStep1');
const stepForm2 = document.getElementById('contactFormStep2');
...
if (stepForm1) {
  stepForm1.addEventListener('submit', (e) => {
    ...
    // Copy email to step 2 form
    const emailStep2 = document.getElementById('email');
    if (emailStep2) {
...
if (stepForm2) {
  ...
  stepForm2.addEventListener('submit', (e) => {
    e.preventDefault();
...
function showStep2() {
  if (stepForm1 && stepForm2) {
    stepForm1.classList.remove('step-form-active');
    stepForm1.classList.add('step-form-hidden');
    stepForm2.classList.remove('step-form-hidden');
    stepForm2.classList.add('step-form-active');

    // Scroll to form
    stepForm2.scrollIntoView({ behavior: 'smooth', block: 'center' });
  }
}

function showStep1() {
  if (stepForm1 && stepForm2) {
    stepForm2.classList.remove('step-form-active');
    stepForm2.classList.add('step-form-hidden');
    stepForm1.classList.remove('step-form-hidden');
    stepForm1.classList.add('step-form-active');
  }
}

// ===================================
// Legacy Form Validation (for old single form)
// ===================================
...
// シンプルなホバー効果はCSSで実装済み
document.addEventListener('DOMContentLoaded', function() {
  // 3Dチルト効果は無効化 - CSSのhoverのみ使用
  const cards = document.querySelectorAll('.problem-card, .target-card, .service-card');

  cards.forEach(card => {
    // 特別な効果なし - CSSのtransitionで十分
  });
});
